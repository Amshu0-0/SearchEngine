...
def most_recent_article(metadata):
    if len(metadata) != 0:
        recent_article = metadata[0]
        recent_timestamp = metadata[0][2]
        for data in metadata:
            if data[2] >= recent_timestamp:
                recent_timestamp = data[2]
                recent_article = data
...
def favorite_author(favorite, metadata):
    for data in metadata:
        author = data[1]
        if author.lower() == favorite.lower():
            return True
    return False
